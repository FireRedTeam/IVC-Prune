...
class DeepseekV2RotaryEmbeddingIVCP(nn.Module):
    def __init__(self, dim, max_position_embeddings=2048, base=10000, device=None):
        super().__init__()
...
class DeepseekV2Attention(nn.Module):
    ...
    def _init_rope(self):
        if self.config.rope_scaling is None:
            ...
            self.rotary_emb = DeepseekV2RotaryEmbeddingIVCP(
    ...
    def forward(
        ...
        **kwargs,
        ...
        compute_value_states = compressed_kv.clone()
        ...
        if not output_pe:
            ...
            v_projected = torch.einsum("blc,hvc->bhlv", compute_value_states, compute_out_absorb)  # (B, H, L, v_head_dim)
            value_weights = torch.matmul(v_projected, v_projected.transpose(2, 3)) / math.sqrt(self.v_head_dim)

            value_weights = nn.functional.softmax(value_weights, dim=-1, dtype=torch.float32).to(v_projected.dtype)


            return attn_output, attn_weights, past_key_value, value_weights, saved_attn_weights, v_projected, (cos, sin)
...
class DeepseekV2DecoderLayer(nn.Module):
    def __init__(self, config: DeepseekV2Config, layer_idx: int):
        ...
        # tiny use_mla false  attn_implementation: mha_eager
    ...
    def forward(
        ...
        **kwargs,
        ...
        if not output_pe:
            ...
            hidden_states, self_attn_weights, present_key_value, value_weights, saved_attn_weights, v_projected, pe = self.self_attn(
                hidden_states=hidden_states,
                attention_mask=attention_mask,
                position_ids=position_ids,
                past_key_value=past_key_value,
        ...
        if output_pe:
            outputs += (value_weights, )
            outputs += (saved_attn_weights, )
...
class DeepseekV2Model(DeepseekV2PreTrainedModel):
    ...
    def _process_detail_images_with_newlines(
        self,
        attention_scores,
        img_h,
        img_w,
        ...
        tokens_per_row = img_w + 1
        
        for row in range(img_h):
            row_start = row * tokens_per_row
            for col in range(img_w):
                idx = row_start + col
                if idx < len(attention_scores):
                    valid_scores.append(attention_scores[idx])
    ...
    def _process_single_image_window(
        ...
        attention_scores_2d = attention_scores.reshape(img_h, img_w)
        
        num_windows_h = img_h // window_h
        num_windows_w = img_w // window_w
    ...
    def _find_best_divisor(self, dimension, target, min_val, max_val):
        ...
        if candidates:
            ...
            return max(min_val, min(target, max_val))
    ...
    def calculate_avg_similarity_for_subset_einsum(
        ...
        sim_slice = torch.einsum('bhqd,bhdk->bhqk', value_states_query, value_states_t) / scale
    ...
    def IVCP_forward_flashattention(
        ...
        cache_position: Optional[torch.LongTensor] = None,
        ivcp_config = None,
        image_pose: Optional = None 
    ) -> Union[Tuple, BaseModelOutputWithPast]:
        IVCP_k = ivcp_config['ivcp_k']
        IVCP_image_token_start_index = ivcp_config['image_start_indices']
        IVCP_image_token_length = ivcp_config['image_token_lengths']
        ...
        if hidden_states.shape[1] == 1:
            new_index = torch.tensor([self.keep_indexs[-1] + 1 ], dtype=self.keep_indexs.dtype, device=self.keep_indexs.device)
        ...
        for layer_idx, decoder_layer in enumerate(self.layers):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                if hidden_states.shape[1] != 1:
                    if layer_idx < IVCP_k-1:
                    ...
                    elif layer_idx == IVCP_k:

                        v_projected = layer_outputs[5]
                        cos, sin = layer_outputs[-1]
                        text_query_indices = torch.arange(
                            IVCP_image_token_start_index + IVCP_image_token_length + 1,
                            seq_length_with_past,
                            device=device
                            )

                        value_attn_text_to_img = self.calculate_avg_similarity_for_subset_einsum(
                            v_projected,
                            text_query_indices,
                            self.head_dim,
                            img_start=IVCP_image_token_start_index,
                            img_len=IVCP_image_token_length
                        )
                        value_attn_text_to_img_mean = torch.mean(value_attn_text_to_img, dim=0)

                        valid_mask = torch.ones_like(value_attn_text_to_img_mean, dtype=torch.bool).to(value_attn_text_to_img_mean.device)
                        exclude_indices = image_pose[2][0][0] + [14,29,44,59,74,89,104,119,134,149,164,179,194,209]
                        valid_mask[exclude_indices] = False

                        valid_attention = value_attn_text_to_img_mean[valid_mask]
                        valid_indices = torch.arange(len(value_attn_text_to_img_mean)).to(value_attn_text_to_img_mean.device)[valid_mask]
                        k = round(len(valid_attention) * 0.01)

                        topk_values, topk_local_indices = valid_attention.topk(k)
                        all_question_top_attention_rank_index = valid_indices[topk_local_indices] + IVCP_image_token_start_index


                        value_attn_A_to_img = self.calculate_avg_similarity_for_subset_einsum(
                            v_projected,
                            all_question_top_attention_rank_index,
                            self.head_dim,
                            img_start=IVCP_image_token_start_index,
                            img_len=IVCP_image_token_length
                        )
                        img_scores_final = torch.cat((value_attn_text_to_img, value_attn_A_to_img), dim=0)
                        ...
                        save_image_cos = cos_mean[IVCP_image_token_start_index: IVCP_image_token_start_index+IVCP_image_token_length]
                        save_image_sin = sin_mean[IVCP_image_token_start_index: IVCP_image_token_start_index+IVCP_image_token_length]

                        global_image_cos = cos_mean[IVCP_image_token_start_index: IVCP_image_token_start_index + 210]
                        global_image_sin = sin_mean[IVCP_image_token_start_index: IVCP_image_token_start_index + 210]

                        detail_images_cos = cos_mean[IVCP_image_token_start_index + 211: IVCP_image_token_start_index+IVCP_image_token_length]
                        detail_images_sin = sin_mean[IVCP_image_token_start_index + 211: IVCP_image_token_start_index+IVCP_image_token_length]

                        cos_top_values, global_cos_top_indices = torch.topk(global_image_cos, k=round(210*0.1))
                        sin_top_values, global_sin_top_indices = torch.topk(global_image_sin, k=round(210*0.1))

                        num_height_tiles = image_pose[1][1]
                        num_width_tiles = image_pose[1][0]

                        num_width = num_width_tiles * 14 + 1
                        num_height = num_height_tiles * 14

                        indices1 = torch.arange(1, num_height + 1, device=device) * num_width - 1 + 211
                        indices2 = torch.arange(14, device=device) * 15 + 14

                        indices3 = torch.tensor([210], device=device)
                        seprate_token_indice = torch.cat([indices1, indices2, indices3])

                        row_indices = torch.arange(num_height, device=detail_images_cos.device).unsqueeze(1)
                        row_offsets = row_indices * num_width

                        blocks_cos = detail_images_cos.reshape(num_height, num_width)
                        blocks_sin = detail_images_sin.reshape(num_height, num_width)

                        cos_top_values, line_cos_top_indices = torch.topk(blocks_cos, k=round(num_width.item()*0.15), dim=-1)
                        sin_top_values, line_sin_top_indices = torch.topk(blocks_sin, k=round(num_width.item()*0.15), dim=-1)
                        original_line_cos_top_indices = row_offsets + line_cos_top_indices
                        original_line_sin_top_indices = row_offsets + line_sin_top_indices
                        ...
                        top_value_related_attention = remaining_sorted_indices[:k]

                        value_related_attention_topk_indices = valid_indices[top_value_related_attention] + IVCP_image_token_start_index


                        foreground_indices = torch.cat((all_question_top_attention_rank_index, value_related_attention_topk_indices))
                        foreground_indices = torch.unique(foreground_indices, sorted=True)


                        local_tokens = self.thumbnail_tokens_to_detailed_blocks(foreground_indices-IVCP_image_token_start_index, image_pose[1])

                        indices_list = [
                            torch.arange(IVCP_image_token_start_index, device=device),
                            original_line_cos_top_indices + IVCP_image_token_start_index + 211,
                            original_line_sin_top_indices + IVCP_image_token_start_index + 211,
                            foreground_indices,
                            torch.tensor(local_tokens, device=device) + IVCP_image_token_start_index,
                            seprate_token_indice + IVCP_image_token_start_index,
                            global_cos_top_indices + IVCP_image_token_start_index,
                            global_sin_top_indices + IVCP_image_token_start_index,
                            torch.arange(IVCP_image_token_start_index + IVCP_image_token_length, seq_length_with_past, device=device)
                        ]
                        
                        keep_indexs = torch.cat(indices_list)

                        keep_indexs = torch.unique(keep_indexs, sorted=True)

                        self.keep_indexs = keep_indexs

                        pruned_hidden_states = hidden_states[:, keep_indexs, :].contiguous()


                        pruned_attention_mask = attention_mask[:, :, keep_indexs, :]
                        pruned_attention_mask = pruned_attention_mask[:, :, :, keep_indexs].contiguous()

                        pruned_position_ids = position_ids[:, keep_indexs].contiguous()
                    ...
                    elif layer_idx < IVCP_k and layer_idx >0:
                        pruned_attention_mask = attention_mask[:, :, :, self.keep_indexs]
                        layer_outputs = decoder_layer(
                            hidden_states,
                            attention_mask=pruned_attention_mask,
                            position_ids=position_ids,
...
class DeepseekV2ForCausalLM(DeepseekV2PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.ivcp_config = config.ivcp_config
        # Initialize weights and apply final processing
        self.post_init()
