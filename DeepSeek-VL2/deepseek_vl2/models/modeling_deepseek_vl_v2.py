...
class DeepseekVLV2ForCausalLM(DeepseekVLV2PreTrainedModel):
    ...
    def __init__(self, config: DeepseekVLV2Config):
        ...
        ivcp_config = getattr(config, 'ivcp_config', None)
        ...
        language_config.ivcp_config = ivcp_config
    ...
    def get_relative_special_token_positions(
        self,
        input_ids: torch.LongTensor,
        images_spatial_crop: torch.LongTensor,
        images_seq_mask: torch.BoolTensor
    ...
    def prepare_inputs_embeds(
        ...
        for idx in range(bs):
            for jdx in range(max_n_images):
                num_width_tiles, num_height_tiles = images_spatial_crop[idx, jdx]
                # print("num_width_tiles", num_width_tiles, num_height_tiles)
                if num_width_tiles == 0 or num_height_tiles == 0:
                    break
