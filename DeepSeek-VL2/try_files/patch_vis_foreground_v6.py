...
class SimpleImageProcessingVisualizer:
    ...
    def get_patch_token_ranges(self, patch_row, patch_col, num_width_tiles, num_height_tiles, local_start_token):
        """获取指定patch的所有token ID"""
        h = w = math.ceil((self.image_size // self.patch_size) / self.downsample_ratio)  # 14
        ...
        # 遍历该patch的每一行
        for local_row in range(h):  # 14行
            # 计算全局行号
            global_row = patch_row * h + local_row
            # 计算该行的起始token ID
            row_start_token = local_start_token + global_row * tokens_per_row
            # 计算该patch在该行的起始列
            patch_start_col = patch_col * w
            # 该patch在该行的token范围
            for local_col in range(w):  # 14列
                token_id = row_start_token + patch_start_col + local_col
                patch_tokens.append(token_id)
    ...
    def add_token_grid_to_patch(self, patch_image, patch_type='local', patch_row=0, patch_col=0, 
        ...
        if patch_type == 'global':
                ...
                for j in range(w + 1):  # 15列
                    if j < w:  # 图像token
                        ...
                        if token_id in attention_map:
                            ...
                            # 使用热力图颜色
                            color_rgb = cm.coolwarm(norm_att)[:3]
            ...
            # 判断是否需要显示特殊token（只在最右边的patch中显示）
            ...
            for i in range(h):  # 14行
                ...
                if should_show_special:
                    ...
                    # 检查特殊token的attention
                    fill_color = None
                    if special_token_id in attention_map:
                        norm_att = (attention_map[special_token_id] - min_att) / att_range
                        color_rgb = cm.coolwarm(norm_att)[:3]
                        fill_color = tuple(int(c * 255) for c in color_rgb)
                    ...
                    if special_token_id in attention_map:
                        att_text = f"{attention_map[special_token_id]:.3f}"
                        att_y = text_y + 10
                        draw.text((text_x, att_y), att_text, fill=text_color, font=font)
    ...
    def visualize_processing_with_attention(self, image_path: str, num_width_tiles: int, num_height_tiles: int, 
        ...
        for i, patch in enumerate(result['local_patches']):
            ...
            # 正确计算这个patch的token列表和attention统计
            patch_tokens = self.get_patch_token_ranges(row, col, num_width_tiles, num_height_tiles, local_start_token)
            
            title = f'Patch ({row},{col})\n{self.image_size}×{self.image_size}'
            title += f'\nTokens: {min(patch_tokens)}-{max(patch_tokens)}'
            title += f'\nTotal: {len(patch_tokens)} tokens'
            
            if attention_data is not None:
                patch_tokens_with_att = sum(1 for tid in topk_idx if tid in patch_tokens)
                if patch_tokens_with_att > 0:
                    title += f'\nAttention: {patch_tokens_with_att} tokens'
                    # 显示该patch中attention的范围
                    patch_attentions = [attention_data[list(topk_idx).index(tid)] for tid in topk_idx if tid in patch_tokens]
                    if patch_attentions:
                        title += f'\nRange: {min(patch_attentions):.3f}-{max(patch_attentions):.3f}'
        ...
        if attention_data is not None:
            ...
            # 显示token ID范围
            main_title += f'\nToken ID range: {min(topk_idx)}-{max(topk_idx)}'
    ...
    def print_attention_analysis(self, npz_path: str, num_width_tiles: int, num_height_tiles: int):
        ...
        global_tokens = h * (w + 1)
        separator_tokens = 1
        local_start_token = global_tokens + separator_tokens
        ...
        for i, token_id in enumerate(topk_idx):
            if i < len(attention_data):
                ...
                if token_id < global_tokens:
                    ...
                    # 分析是哪个patch
                    local_token_id = token_id - local_start_token
                    tokens_per_row = num_width_tiles * w + 1
                    local_row = local_token_id // tokens_per_row
                    local_col = local_token_id % tokens_per_row
                    
                    if local_col < num_width_tiles * w:  # 确保是图像token
                        patch_row = local_row // h
                        patch_col = local_col // w
                        
                        if 0 <= patch_row < num_height_tiles and 0 <= patch_col < num_width_tiles:
                            patch_key = (patch_row, patch_col)
                            if patch_key not in patch_stats:
                                patch_stats[patch_key] = []
                            patch_stats[patch_key].append((token_id, att_val))
        
        print(f"Total selected tokens: {len(topk_idx)}")
        print(f"Token ID range: {min(topk_idx)} - {max(topk_idx)}")
        ...
        # 按patch统计
        print(f"\nPer-patch attention statistics:")
        for (patch_row, patch_col), tokens_with_att in patch_stats.items():
            attentions = [att for _, att in tokens_with_att]
            token_ids = [tid for tid, _ in tokens_with_att]
            print(f"  Patch ({patch_row},{patch_col}): {len(tokens_with_att)} tokens")
            print(f"    Token ID range: {min(token_ids)} - {max(token_ids)}")
            print(f"    Attention range: {min(attentions):.4f} - {max(attentions):.4f}")
            print(f"    Mean attention: {np.mean(attentions):.4f}")
...
if __name__ == "__main__":
    ...
    npz_path = "/mnt/public/usr/sunzhichao/DeepSeek-VL2/try_files/layer17_fastv_attention_vas_new_v2.npz"
    ...
    result = visualizer.visualize_processing_with_attention(
        ...
        height_tiles,
        npz_path=npz_path,
        save_path=f"/mnt/public/usr/sunzhichao/DeepSeek-VL2/try_files/new_10_pe_visualization_with_attention_{width_tiles}x{height_tiles}_fixed.png"
    )
