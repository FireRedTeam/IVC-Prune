...
class Qwen2VLModel(Qwen2VLPreTrainedModel):
    ...
    def compute_attention_weights(self, hidden_states, attention_mask, position_ids, position_embeddings, layer_idx, past_key_value=None, cache_position=None):
        """
        手动计算注意力权重（参考Qwen2_5_VLAttention的实现）
        """
        ...
        # Apply rotary position embedding（参考Qwen2_5_VLAttention）
        cos, sin = position_embeddings
        
        # 处理position_ids的维度（参考Qwen2_5_VLAttention）
        ...
        # 处理past_key_value cache（参考Qwen2_5_VLAttention）
        if past_key_value is not None:
            ...
            if hasattr(past_key_value, 'key_cache') and len(past_key_value.key_cache) > layer_idx:
                ...
                if cached_key is not None and cached_value is not None:
                    # 拼接新的key/value states和cached states
        ...
        attn_weights = None
        attn_output = None
        # value_sim = torch.matmul(value_states, value_states.transpose(2, 3)) / math.sqrt(self_attn.head_dim)
        # value_weights = nn.functional.softmax(value_sim, dim=-1, dtype=torch.float32).to(value_states.dtype)



        # Handle GQA (Grouped Query Attention)（参考Qwen2_5_VLAttention）
        ...
        return attn_output, attn_weights, value_weights, value_states

        # Compute attention scores（参考Qwen2_5_VLAttention）
        attn_weights = torch.matmul(query_states, key_states.transpose(2, 3)) / math.sqrt(self_attn.head_dim)


        # Apply attention mask（参考Qwen2_5_VLAttention）
        ...
        # Fix precision issues in Qwen2-VL float16 inference（参考Qwen2_5_VLAttention）
        if query_states.dtype == torch.float16:
            attn_weights = torch.where(torch.isinf(attn_weights), torch.zeros_like(attn_weights), attn_weights)
        
        # Apply softmax（参考Qwen2_5_VLAttention）
        attn_weights = nn.functional.softmax(attn_weights, dim=-1, dtype=torch.float32).to(query_states.dtype)
        attn_weights = nn.functional.dropout(attn_weights, p=self_attn.attention_dropout, training=self.training)



        hidden_states = hidden_states.view(bsz, q_len, -1, self_attn.head_dim).transpose(1, 2)

        hidden_weights = torch.matmul(hidden_states, hidden_states.transpose(2, 3)) / math.sqrt(self_attn.head_dim)
        hidden_weights = nn.functional.softmax(hidden_weights, dim=-1, dtype=torch.float32).to(query_states.dtype)



        # Compute attention output（参考Qwen2_5_VLAttention）
        attn_output = torch.matmul(attn_weights, value_states)

        # 检查输出尺寸（参考Qwen2_5_VLAttention）
        if attn_output.size() != (bsz, self_attn.num_heads, q_len, self_attn.head_dim):
            raise ValueError(
                f"`attn_output` should be of size {(bsz, self_attn.num_heads, q_len, self_attn.head_dim)}, but is"
                f" {attn_output.size()}"
            )

        # Reshape back（参考Qwen2_5_VLAttention）
        attn_output = attn_output.transpose(1, 2).contiguous()
        attn_output = attn_output.reshape(bsz, q_len, -1)
        
        # Output projection（参考Qwen2_5_VLAttention）
    ...
    def compute_attention_weights_fastv(self, hidden_states, attention_mask, position_ids, position_embeddings, layer_idx, past_key_value=None, cache_position=None):
        """
        手动计算注意力权重（参考Qwen2_5_VLAttention的实现）
        """
        decoder_layer = self.layers[layer_idx]
        ...
        # Compute attention scores（参考Qwen2_5_VLAttention）
        attn_weights = torch.matmul(query_states[:, :, -1, :].unsqueeze(2), key_states.transpose(2, 3)) / math.sqrt(self_attn.head_dim)
        ...
        attn_weights = torch.mean(attn_weights, dim=1)[0][0]
        return None, attn_weights, None, None
    ...
    def fastv_forward_flashattention(
        self,
        input_ids: Optional[torch.LongTensor] = None,
        attention_mask: Optional[torch.Tensor] = None,
        position_ids: Optional[torch.LongTensor] = None,
        ...
        image_token_start_indexes  = None,
    ) -> Union[Tuple, BaseModelOutputWithPast]:
        ...
        if inputs_embeds.shape[1] != 1:
            FASTV_image_token_start_index = image_token_start_indexes[0].item()
            FASTV_image_token_length = image_token_lengths[0].item()
        else:
            FASTV_image_token_start_index = 0
            FASTV_image_token_length = 0
        output_attentions = output_attentions if output_attentions is not None else self.config.output_attentions
        output_hidden_states = (
            output_hidden_states if output_hidden_states is not None else self.config.output_hidden_states
        )
        ...
        length_ids = hidden_states.shape[1]
        if hasattr(self, 'last_token_index') and self.last_token_index >= length_ids:
            self.last_token_index = length_ids - 1

        device = self.device
        seq_length_with_past = past_seen_tokens + inputs_embeds.shape[1]
        
        # 用于存储第FASTV_k-1层的attention weights
        critical_layer_attention = None
        for layer_idx, decoder_layer in enumerate(self.layers):
            if output_hidden_states:
                all_hidden_states += (hidden_states,)
            # 设置当前层使用的mask和position embeddings
            if use_cache:
                if hidden_states.shape[1] != 1:
                    if layer_idx < FASTV_k:
                        ...
                        # 使用之前存储的attention weights进行token剪枝
                        # last_layer_attention = critical_layer_attention
                        # last_layer_attention_avg = torch.mean(last_layer_attention, dim=1)[0]
                        # last_layer_attention_avg_last_tok = last_layer_attention_avg[-1]
                        # print("layer_idx", layer_idx, FASTV_r, FASTV_image_token_start_index, FASTV_image_token_length)
                        last_layer_attention_avg_last_tok = critical_layer_attention
                        ...
                        keep_indexs = torch.cat((
                            torch.arange(FASTV_image_token_start_index, device=device), 
                            top_attention_rank_index,
                            torch.arange(FASTV_image_token_start_index+FASTV_image_token_length, seq_length_with_past, device=device)
                        ))

                        keep_indexs = torch.unique(keep_indexs, sorted=True)
                        # print("keep_indexs", keep_indexs.shape, seq_length_with_past, org_keep_indexs.shape)
                        # print("keep_indexs", keep_indexs.shape, seq_length_with_past, len(keep_indexs)/seq_length_with_past)
                        ...
                        cos, sin = position_embeddings
                        pruned_cos = cos[:, :, keep_indexs, :]
                        pruned_sin = sin[:, :, keep_indexs, :]
                        current_position_embeddings = pruned_cos, pruned_sin

                        current_cache_position = cache_position[keep_indexs]
                ...
                if layer_idx < FASTV_k:
                    ...
                    # 非cache模式下的token剪枝
                    if critical_layer_attention is not None:
                        last_layer_attention = critical_layer_attention
                        last_layer_attention_avg = torch.mean(last_layer_attention, dim=1)[0]
                        last_layer_attention_avg_last_tok = last_layer_attention_avg[getattr(self, 'last_token_index', -1)]
                        last_layer_attention_avg_last_tok_image = last_layer_attention_avg_last_tok[FASTV_image_token_start_index:FASTV_image_token_start_index+FASTV_image_token_length]
                        top_attention_rank_index = last_layer_attention_avg_last_tok_image.topk(round(FASTV_image_token_length*(1-FASTV_r))).indices + FASTV_image_token_start_index
            ...
            # 对于第FASTV_k-1层，需要手动计算attention weights
            if layer_idx == FASTV_k - 1 and critical_layer_attention is None and hidden_states.shape[1] != 1:
                # 手动计算attention weights

                residual = hidden_states
                normalized_hidden_states = decoder_layer.input_layernorm(hidden_states)

                attn_output, attn_weights, _, _ = self.compute_attention_weights_fastv(
                    normalized_hidden_states, 
                    current_attention_mask, 
                    position_ids, 
                    current_position_embeddings, 
                    layer_idx,
                    past_key_values,  # 传递past_key_value
                    current_cache_position  # 传递cache_position
                )
                critical_layer_attention = attn_weights
    ...
    def calculate_avg_similarity_for_subset(
        self, 
        ...
        value_states_t = value_states.transpose(2, 3)
        ...
        # 平均后得到 (S_all,)
        final_score_vector = torch.mean(weights_slice, dim=(0, 1, 2))

        return final_score_vector


    def fastv_forward_flashattention_find_forground_plan_last(
        self,
        input_ids: Optional[torch.LongTensor] = None,
        attention_mask: Optional[torch.Tensor] = None,
        position_ids: Optional[torch.LongTensor] = None,
        ...
        for layer_idx, decoder_layer in enumerate(self.layers):
                ...
                if hidden_states.shape[1] != 1:
                    if layer_idx < 16:
                        ...
                        text_query_indices = torch.arange(
                            FASTV_image_token_start_index + FASTV_image_token_length + 2, 
                            seq_length_with_past, 
                            device=device
                        )

                        value_attn_all_quetsion = self.calculate_avg_similarity_for_subset(
                            critical_value_states,
                            text_query_indices,
                            head_dim
                        )

                        value_attn_all_quetsion_tok_image = value_attn_all_quetsion[FASTV_image_token_start_index:FASTV_image_token_start_index+FASTV_image_token_length]
                        top_attention_rank_index = value_attn_all_quetsion_tok_image.topk(round(FASTV_image_token_length*0.01)).indices
                        
                        new_dim = torch.cat((top_attention_rank_index+FASTV_image_token_start_index, torch.arange(FASTV_image_token_start_index+FASTV_image_token_length+2, seq_length_with_past, device=device)))

                        value_related_attention = self.calculate_avg_similarity_for_subset(
                            critical_value_states,
                            new_dim,
                            head_dim
                        )

                        value_related_attention_tok_image = value_related_attention[FASTV_image_token_start_index:FASTV_image_token_start_index+FASTV_image_token_length]

                        top_value_related_attention = value_related_attention_tok_image.topk(round(FASTV_image_token_length*0.4)).indices + FASTV_image_token_start_index


                        cos, sin = position_embeddings
                        cos_mean = cos[:, 0, FASTV_image_token_start_index: FASTV_image_token_start_index+FASTV_image_token_length, ].mean(dim=(0, 2))
                        sin_mean = sin[:, 0, FASTV_image_token_start_index: FASTV_image_token_start_index+FASTV_image_token_length, ].mean(dim=(0, 2))
                        topk_index_1 = cos_mean.topk(round(FASTV_image_token_length*0.1)).indices + FASTV_image_token_start_index 
                        topk_index_2 = sin_mean.topk(round(FASTV_image_token_length*0.1)).indices + FASTV_image_token_start_index 
                        pe_index = torch.cat((topk_index_1, topk_index_2))


                        keep_indexs = torch.cat((
                            torch.arange(FASTV_image_token_start_index, device=device), 
                            top_attention_rank_index+FASTV_image_token_start_index,
                            top_value_related_attention,
                            pe_index,
                            torch.arange(FASTV_image_token_start_index+FASTV_image_token_length, seq_length_with_past, device=device)
                        ))
            ...
            # 对于第FASTV_k-1层，需要手动计算attention weights
            if layer_idx == 15 and hidden_states.shape[1] != 1:
                # 手动计算attention weights

                residual = hidden_states
                normalized_hidden_states = decoder_layer.input_layernorm(hidden_states)

                attn_output, attn_weights, value_weights, value_states = self.compute_attention_weights(
                    normalized_hidden_states, 
                    current_attention_mask, 
                    position_ids, 
                    current_position_embeddings, 
                ...
                critical_layer_attention = attn_weights
                critical_value_weights = value_weights
                critical_value_states = value_states
...
class Qwen2VLForConditionalGeneration(Qwen2VLPreTrainedModel, GenerationMixin):
    ...
    def get_image_token_length(self, input_ids):
        ...
        for seq in input_ids:
            ...
            if len(start_indices) == 0:
                image_start_indices.append(None)
            else:
                image_start_indices.append(start_indices[0] + 1)

        return image_token_lengths, image_start_indices
    ...
    def forward(
        ...
        return_dict = return_dict if return_dict is not None else self.config.use_return_dict
        image_token_lengths, image_token_start_indexes = self.get_image_token_length(input_ids)
        ...
        if self.config.fastv_config is not None:
            # outputs = self.model.fastv_forward_flashattention(
            #     input_ids=None,
            #     position_ids=position_ids,
            #     attention_mask=attention_mask,
            #     past_key_values=past_key_values,
            ...
            #     cache_position=cache_position,
            #     fastv_config=self.config.fastv_config,
            #     image_token_lengths=image_token_lengths,
            #     image_token_start_indexes=image_token_start_indexes
            # )

            outputs = self.model.fastv_forward_flashattention_find_forground_plan_last(
                input_ids=None,
                position_ids=position_ids,
                attention_mask=attention_mask,
                past_key_values=past_key_values,
